"""Training pipeline for the LBMDiTJointDDTAgent on robomimic.

DDT-trunk variant of the joint pipeline:
- Encoder/decoder width split inside the joint trunk (per-token AdaLN cond).
- Encoder is trainable end-to-end (warm-start from IDM checkpoint optional).
- A learnable LayerNorm normalizes the FM target embedding (E2E pattern).
- Stop-grad on the FM target side blocks the encoder from receiving a
  shortcut gradient through the target.
- State token and action chunk can have **independent** flow times
  (``optimization.joint_decouple_t``); inference uses ``joint_t_schedule``
  to walk a (t_state, t_action) trajectory through [0, 1]^2.

Reuses ``train`` and ``eval`` from the E2E training pipeline; swaps in the
DDT agent.
"""
...
from mip.agent_lbmdit_joint_ddt import LBMDiTJointDDTAgent  # noqa: E402
