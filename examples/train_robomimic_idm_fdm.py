"""IDM training pipeline with auxiliary forward-dynamics (FDM) loss.

Trains an IDM (LBMDiTIDMv2) jointly with a forward-dynamics auxiliary head
that predicts the goal embedding from (summarized obs, projected clean
actions). The FDM loss backprops into the encoder, shaping it to be
forward-predictable in addition to the IDM's controllability signal.
...
``train_robomimic_idm.py``. Clean actions are forwarded through to the FDM
head inside the agent.
"""
...
def train(config: Config, dataset, agent, logger, resume_state=None):
    """IDM+FDM training loop.

    Stacks obs and goal_obs into ``(B, To+1, ...)`` per key and forwards
    ``(act, obs, delta_t)`` to the agent. The agent encodes once and runs both
    the IDM flow loss and the FDM auxiliary internally.
    """
            ...
            with timed("preprocess", perf_times):
                ...
                if config.task.obs_type != "image":
                    raise NotImplementedError(
                        "IDM+FDM training currently only supports image observations"
                    )

                # Stack obs (To frames) + goal (1 frame) -> (B, To+1, ...) per key.
                obs_batch = batch["obs"]
                ...
                for k in obs_batch:
                    ...
                    goal = goal_batch[k].to(config.optimization.device)  # (B, 1, ...)
                    obs_dict[k] = torch.cat([obs, goal], dim=1)  # (B, To+1, ...)
...
def main(config):
    """Main pipeline for IDM+FDM training."""
    ...
    if config.mode == "train":
        ...
        raise ValueError(
            f"IDM+FDM pipeline only supports mode=train; got mode={config.mode}"
        )
