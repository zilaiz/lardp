"""Training pipeline for the DeltaPredictorDDTNSAgent on robomimic.

DDT-NS predictor that targets the goal *delta* (z_goal − z_last_obs) instead
of the absolute goal embedding. Pairs with a delta-cond IDM checkpoint
(``LBMDiTIDMv2Delta``) loaded via ``optimization.idm_checkpoint_path``.

Reuses ``train`` and ``eval`` from the v3 goal-predictor pipeline — the
...
from mip.agent_delta_predictor_ddt_ns import DeltaPredictorDDTNSAgent  # noqa: E402
...
def main(config):
    ...
    agent = DeltaPredictorDDTNSAgent(config)
    resume_state = None
    if config.optimization.model_path and config.optimization.model_path != "None":
        loguru.logger.info(
            f"Loading DeltaPredictorDDTNS from {config.optimization.model_path}"
        )
