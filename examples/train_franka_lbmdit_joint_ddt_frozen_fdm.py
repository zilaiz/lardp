"""Franka LBMDiTJointDDTFrozen training pipeline with a BYOL-FDM-pretrained encoder.

Same DDT-trunk joint flow matching as ``train_franka_lbmdit_joint_ddt_frozen.py``
but loads the encoder from an ``FDMAgent`` checkpoint instead of an IDM
checkpoint. Honors ``optimization.joint_use_encoder_ema_for_init`` to pick
the BYOL EMA target over the online encoder.

Mirrors examples/train_franka_goal_predictor_dit.py in dataset/eval plumbing
but swaps in ``LBMDiTJointDDTFrozenFDMAgent``:
...
from mip.agent_lbmdit_joint_ddt_frozen_fdm import LBMDiTJointDDTFrozenFDMAgent
...
def compute_val_loss(
    agent: LBMDiTJointDDTFrozenFDMAgent,
