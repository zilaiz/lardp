"""Franka o2g goal-predictor training pipeline (real-robot, no sim env).

Mirrors examples/train_franka_goal_predictor_dit.py but uses the o2g pipeline:

- Flows from the frozen IDM's obs_summarizer token directly to the raw goal
  embedding via a residual-MLP velocity net. No latent normalization, no
  Gaussian-noise prior, no action-regularization. Optional VITA-style
  consistency loss (full-ODE goal integration -> action FM through frozen IDM)
  controlled by ``optimization.consistency_weight``.
- Loads the IDM dataset (with goal_obs) from one or more HDF5 files.
- Loads a frozen IDM checkpoint trained by ``train_franka_idm_fdm.py``
  (must be ``lbmidm_v2`` so the o2g agent can reuse the obs_summarizer).
...
- No simulator, no rollout eval. Inline eval is replaced with periodic val
  loss (state flow loss + optional unscaled consistency loss) on the
  held-out primary HDF5's val split.
"""
...
from mip.agent_goal_predictor_o2g import GoalPredictorO2GAgent
...
def compute_val_loss(
    agent: GoalPredictorO2GAgent,
    val_loader: torch.utils.data.DataLoader,
    config: Config,
    max_batches: int = 50,
) -> dict[str, float]:
    """Val state-flow loss (and optional unscaled consistency loss) on EMA goal-net.

    Mirrors the agent's update() math but uses the EMA flow map.
    """
    agent.eval()
    cfg = config.optimization

    state_losses: list[float] = []
    consistency_losses: list[float] = []  # unscaled
    use_consistency = agent._consistency_weight > 0
    ...
    for i, batch in enumerate(val_loader):
        ...
        z_goal = agent._inner_encoder(goal_obs, None)
        x0 = agent._make_x0(z_t)
        x1 = z_goal

        B = x1.shape[0]
        device = x1.device
        cond = agent._make_cond(z_t)

        # State flow
        t_flow = torch.empty(B, device=device).uniform_(0, 1)
        xt = agent.goal_interpolant.calc_It(t_flow, x0, x1)
        ut = agent.goal_interpolant.calc_It_dot(t_flow, x0, x1)
        v_pred, _ = agent.goal_net_ema(xt, t_flow, t_flow, cond)
        state_losses.append(
            float(torch.mean(get_norm(v_pred - ut, cfg.norm_type)).item())
        )

        # Consistency (optional, monitored only when enabled)
        if use_consistency:
            # Replay the same logic as agent._integrate_goal_with_grad but on EMA fm
            g_s = x0
            t_sched = np.linspace(0, 1, agent._consistency_num_steps + 1)
            for k in range(agent._consistency_num_steps):
                s = torch.full((B,), t_sched[k], device=device)
                v = agent.goal_flow_map_ema.get_velocity(s, g_s, cond)
                g_s = g_s + v * (t_sched[k + 1] - t_sched[k])
            obs_emb = torch.cat([z_t, g_s], dim=1)
            t_act = torch.empty(B, device=device).uniform_(0, 1)
            act_0 = torch.randn_like(act)
            act_t = agent.action_interpolant.calc_It(t_act, act_0, act)
            act_t_dot = agent.action_interpolant.calc_It_dot(t_act, act_0, act)
    ...
    agent.train()
    out = {
        "val_state_flow_loss": float(np.mean(state_losses)) if state_losses else float("nan"),
    }
    if consistency_losses:
        out["val_consistency_loss_unscaled"] = float(np.mean(consistency_losses))
    return out
    ...
    for n_gradient_step in range(start_step, config.optimization.gradient_steps):
        ...
        if ((n_gradient_step + 1) % config.log.eval_freq) == 0:
            ...
            if val_loader is not None:
                loguru.logger.info("Computing val state-flow / consistency loss...")
...
def main(config: Config):
    ...
    if config.task.obs_type == "image":
        ...
        raise NotImplementedError(
            "Franka goal predictor o2g training only supports image obs"
        )
    ...
    # --- agent (loads frozen IDM internally; requires lbmidm_v2 IDM) ---
    agent = GoalPredictorO2GAgent(config)
    resume_state = None
    if config.optimization.model_path and config.optimization.model_path != "None":
        loguru.logger.info(
            f"Loading goal predictor o2g from {config.optimization.model_path}"
        )
    ...
    if config.mode == "train":
        ...
        raise ValueError(
            f"Franka goal predictor o2g pipeline only supports mode='train' (got {config.mode!r})"
        )
