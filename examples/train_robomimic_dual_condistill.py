"""Training pipeline for dual-network condistill on robomimic dataset.

Adapted from train_robomimic_condistill.py.
No dropout annealing — teacher and student are fully separate trainable networks.
"""
...
from mip.agent_dual_condistill import TrainingAgentDualCondistill  # noqa: E402
...
def train(config: Config, envs, dataset, agent, logger, resume_state=None, dino_extractor=None):
    """Standalone training function for dual-network condistill."""
...
def eval(config: Config, envs, dataset, agent, logger, num_steps=1, dino_extractor=None):
    """Standalone inference function to evaluate a trained agent."""
    episode_rewards = []
    episode_steps = []
    episode_success = []
    episode_kit_success = []
            ...
            with timed("normalize", inference_times):
                if config.task.obs_type == "state":
                    ...
                    if encoder_type == "dino":
                        ...
                        obs = {}
                        for k in obs_raw:
                            obs[k] = obs_raw[k].astype(np.float32)
...
def main(config):
    ...
    agent = TrainingAgentDualCondistill(config)
    resume_state = None
