"""Training pipeline for goal predictor through frozen IDM on robomimic dataset.

Based on train_robomimic_idm.py. Key difference: instead of training the IDM,
we load a pretrained frozen IDM and train a goal predictor MLP that predicts
goal embeddings. The goal predictor is optimized by backpropagating
action-matching loss through the frozen IDM.
"""
...
from mip.agent_goal_predictor import GoalPredictorAgent  # noqa: E402
...
def train(config: Config, envs, dataset, agent, logger, resume_state=None):
    """Goal predictor training function."""
        ...
        with timed("total_step", perf_times):
            ...
            with timed("preprocess", perf_times):
                ...
                if config.task.obs_type == "image":
                    ...
                    # Goal obs: (B, 1, ...) — separate, for state-matching loss
                    ...
                    raise NotImplementedError(
                        "Goal predictor training currently only supports image observations"
                    )
...
def eval(config: Config, envs, dataset, agent, logger, num_steps=1):
    """Evaluate the goal predictor + frozen IDM as a coupled policy.

    Unlike IDM eval, no goal frame needs to be appended — the
    GoalPredictorEncoderWrapper handles goal prediction internally.
    """
    episode_rewards = []
    episode_steps = []
    episode_success = []
        ...
        while t < config.task.max_episode_steps:
            with timed("normalize", inference_times):
                if config.task.obs_type == "image":
                    ...
                    raise NotImplementedError(
                        "Goal predictor eval currently only supports image observations"
                    )
...
def main(config):
    """Main pipeline for goal predictor training."""
    ...
    agent = GoalPredictorAgent(config)
    resume_state = None

    if config.optimization.model_path and config.optimization.model_path != "None":
        loguru.logger.info(f"Loading goal predictor from {config.optimization.model_path}")
