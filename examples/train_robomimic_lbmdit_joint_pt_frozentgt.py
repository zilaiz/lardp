"""Training entry for LBMDiTJointPTFrozenTargetAgent on robomimic.

State-representation ablation of the joint_pt pipeline: identical to
``train_robomimic_lbmdit_joint_pt.py`` (reuses the E2E ``train`` / ``eval``
loops) but swaps the agent so the FM state-flow target is a **frozen external
encoder** (``dp_checkpoint_path``) z-scored by precomputed per-dim goal stats
(``goal_stats_path``), instead of the trainable ``target_ln`` self-target. The
input encoder and the ``joint_state_loss_to_encoder`` knob are unchanged.

Export the goal stats first with ``scripts/compute_goal_stats_dp.py`` (point
``--dp_checkpoint`` at the same ckpt as ``dp_checkpoint_path``), then pass
``optimization.goal_stats_path=<stats.pt>`` and
``optimization.dp_checkpoint_path=<ckpt.pt>`` to this script.
"""
