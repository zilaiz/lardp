"""Training pipeline for the single-stage LBMDiTJoint agent on robomimic.

The joint trunk flow-matches (next_state_embedding, action_chunk) under AdaLN
conditioning on (time, encoded current obs, optimality). The encoder is
loaded frozen from a pretrained IDM checkpoint — same setup as
``train_robomimic_goal_predictor_dit.py``.

Optimality labeling: by default every sample is tagged as expert (slot 0).
When training on mixed expert+play data, replace ``_make_optimality(...)``
with a labeling scheme that returns slot 1 for play samples — that's the
single hook for CFG-style expert/play conditioning.
"""
...
from mip.agent_lbmdit_joint import LBMDiTJointAgent  # noqa: E402
...
def main(config):
    ...
    agent = LBMDiTJointAgent(config)
    resume_state = None
    if config.optimization.model_path and config.optimization.model_path != "None":
        loguru.logger.info(
            f"Loading LBMDiTJoint from {config.optimization.model_path}"
        )
