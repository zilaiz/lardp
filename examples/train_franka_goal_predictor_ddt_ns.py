"""Franka goal-predictor DDT-NS training pipeline (real-robot, no sim env).

Mirrors examples/train_franka_goal_predictor_dit.py but swaps:
  - Agent: GoalPredictorDDTNSAgent (DDT trunk + noise-shift) instead of
    GoalPredictorDiTAgent.
  - val-loss path: samples t_flow from the same shift-warped distribution
    the agent uses at training (so the val curve is comparable to train).

Everything else (dataset construction, train loop, periodic save, IDM-
normalizer reuse) is identical.
"""
...
def compute_val_loss(
    agent: GoalPredictorDDTNSAgent,
    val_loader: torch.utils.data.DataLoader,
    config: Config,
    max_batches: int = 50,
) -> dict[str, float]:
    """State-flow + (one-step) action-reg loss on the val set, EMA goal-DDT.

    t_flow is drawn from the same noise-shifted distribution the agent uses
    at training time (``goal_t_dist`` -> ``_apply_t_shift``), clamped to
    ``[goal_t_eps, 1 - goal_t_eps]``. Without that, the val curve would be
    measured on a different t-distribution than the train curve and the two
    ...
    for i, batch in enumerate(val_loader):
        ...
        t_base = _sample_base_t(
            (B,), device, eps, 1.0 - eps,
            agent._t_dist, agent._t_dist_mu, agent._t_dist_sigma,
        )
        t_flow = _apply_t_shift(t_base, agent._t_shift)
        ...
        # EMA goal-DDT for stable val-side velocity.
        ...
        # One-step action-reg loss for monitoring (matches K=1 shortcut in
        # the parent val flow; cheaper than K-step refinement at val time).
    ...
    out = {
        "val_state_flow_loss": (
            float(np.mean(state_losses)) if state_losses else float("nan")
        ),
    }
    if action_losses:
...
def main(config: Config):
    ...
    if config.task.obs_type == "image":
        ...
        raise NotImplementedError(
            "Franka goal predictor DDT-NS training only supports image obs"
        )
    ...
    if config.optimization.model_path and config.optimization.model_path != "None":
        loguru.logger.info(
            f"Loading goal predictor DDT-NS from {config.optimization.model_path}"
        )
