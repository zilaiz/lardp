...
class _FinalLayer(nn.Module):
    def __init__(self, hidden_size, out_size, cls_token_dim):
        super().__init__()
    ...
    def forward(self, x, t, cond, cls=None):
        # process the conditioning vector first
        # x shape: (Ta, b, d_model)
        cond = torch.mean(cond, axis=0)
        cond = cond + t
        ...
        if cls is None:
            ...
            cls_token = self.linear_cls(x[0, :]).unsqueeze(0)
            x = self.linear(x[1:, :])
            return x.transpose(0, 1), cls_token.transpose(0, 1)
...
class SudeepDiTREG(BaseNetwork):
    def __init__(
        self,
        act_dim: int,
        Ta: int,
        ...
        # Output layers
        cls_token_dim = z_dims[0]
        self.final_layer = _FinalLayer(d_model, act_dim, cls_token_dim)
        self.final_layer.reset_parameters()

        self.cls_projectors2 = nn.Linear(in_features=cls_token_dim, out_features=d_model, bias=True)
    ...
    def forward(
        ...
        if cls_token is not None: # (b, 1, z_dim)
...
def test_sudeepditrepa():
    ...
    y, scalar_out, zs_tilde, y_cls = model(x, s, t, condition)
    ...
    y_no_cond, _, zs_no_cond, y_cls_no_cond = model(x, s, t, None)
    ...
    y1, _, _, _ = model_no_time(x, s, t, condition)
    # Test with different time values - should give same output
    y2, _, _, _ = model_no_time(
