"""DiT network with cross-attention decoder.
...
class SudeepDiTOGXAttn(BaseNetwork):
    """DiT with cross-attention decoder.

    Encoder: self-attention over observation tokens.
    ...
    def forward(
        ...
        # Final output
        y = self.final_layer(y_tokens, time_emb, enc_out)

        scalar = None
        return y, scalar


def test_sudeepdit_og_xattn():
    print("=" * 50)
    print("Testing SudeepDiTOGXAttn")
    print("=" * 50)
    ...
    model_nt = SudeepDiTOGXAttn(
        act_dim=act_dim, Ta=Ta, obs_dim=obs_dim, To=To,
        d_model=d_model, n_heads=4, depth=2, disable_time_embedding=True,
    )
    y1, _ = model_nt(x, s, t, condition)
    y2, _ = model_nt(x, torch.randn(batch_size), torch.randn(batch_size), condition)
