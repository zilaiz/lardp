"""Goal Predictor DDT Agent: trains an encoder-decoder DiT (DDT head) goal predictor through a frozen IDM.

The frozen IDM (encoder + flow_map) is loaded from a pretrained checkpoint.
A DiT with DDT head generates goal embeddings via flow matching in the frozen
encoder's embedding space. Training uses state flow loss only (velocity matching
in standardized goal embedding space). No action regularization / align_depth.
...
from mip.networks.goal_predictor_dit import GoalPredictorDiTDDT
from mip.samplers import ode_sampler
from mip.torch_utils import at_least_ndim, report_parameters


class GoalPredictorDDTEncoderWrapper(BaseEncoder):
    """Wraps frozen encoder + goal DDT flow map into a single encoder interface.

    At inference, this wrapper:
    1. Encodes obs with frozen encoder -> z_t
    2. Runs goal DDT ODE to generate goal embedding in normalized space
...
class GoalPredictorDDTAgent:
    """Agent that trains a DDT-based goal predictor through a frozen pretrained IDM.

    Uses encoder-decoder DiT architecture (faithful to RAE's DiTwDDTHead):
    both encoder and decoder receive the same noisy goal, encoder builds a rich
    representation conditioned on time+obs, which conditions the decoder via AdaLN.
    """
    ...
    def __init__(self, config: Config):
        ...
        if idm_path is None:
            raise ValueError("idm_checkpoint_path must be set for GoalPredictorDDTAgent")
        ...
        self.goal_dit = GoalPredictorDiTDDT(
            act_dim=enc_out_dim,
            Ta=1,
            obs_dim=enc_out_dim,
        ...
        report_parameters(self.goal_dit, model_name="Goal Predictor DDT")
        ...
        self.wrapper_encoder = GoalPredictorDDTEncoderWrapper(
            self._inner_encoder,
            self.goal_flow_map,
        ...
        self.wrapper_encoder_ema = GoalPredictorDDTEncoderWrapper(
            self._inner_encoder,
        ...
        # --- Optimizer (only goal DDT params) ---
    ...
    def update(
        ...
        """Training step (state flow loss only).

        Args:
            act: (B, horizon, act_dim) expert actions (unused, kept for interface compat)
        ...
        # Forward through goal DDT
        v_pred, _ = self.goal_dit(x_t, t_flow, t_flow, z_t)
    ...
    def sample(
        ...
        """Sample actions: encode obs -> generate goal via DDT ODE -> run IDM sampler.
        ...
        Args:
            ...
            use_ema: whether to use EMA goal DDT
        """
        if num_steps >= 1:
    ...
    def save(self, path: str, training_state: dict = None):
        """Save goal predictor DDT checkpoint."""
        checkpoint = {
    ...
    def load(self, path: str, load_optimizer: bool = False):
        """Load goal predictor DDT checkpoint.
    ...
    def eval(self):
        """Set goal DDT to eval mode. IDM is always in eval."""
        self.goal_dit.eval()
        self.goal_dit_ema.eval()

    def train(self):
        """Set goal DDT to train mode. IDM stays frozen/eval."""
        self.goal_dit.train()
