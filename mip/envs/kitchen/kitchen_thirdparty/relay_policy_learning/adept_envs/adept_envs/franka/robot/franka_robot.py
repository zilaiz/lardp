...
class Robot(base_robot.BaseRobot):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_hardware:
            if franka_interface == "":
                ...
                cprint(
                    f"Initializing {self.robot_name} Hardware (Status:{self.franka.okay(self.robot_hardware_dof)})",
                    "white",
                    "on_grey",
                )
            else:
    ...
    def get_obs_from_cache(self, env, index=-1):
        assert (index >= 0 and index < self.observation_cache_maxsize) or (
            ...
            f"cache index out of bound. (cache size is {self.observation_cache_maxsize:2d})"
        )
...
class Robot_PosAct(Robot):  # noqa: N801
...
class Robot_VelAct(Robot):  # noqa: N801
    # enforce velocity sepcs.
    # ALERT: This depends on previous observation. This is not ideal as it breaks MDP addumptions. Be careful
